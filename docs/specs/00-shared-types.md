...
| TEST-02 | PRED-03, PRED-04, PRED-05, PRED-06 | The predicates are driven with hand-built plain objects; no error instance and no HTTP call is involved. Every rejected input fails for exactly one reason, so each names the clause it isolates. PRED-03 accepts `{ success: true, result: 1 }`, `{ success: false, result: 1 }`, `{ success: true, result: undefined }`, and the prototype case `Object.create({ success: true, result: 1 })`. PRED-04 rejects, one clause each: `null`; `"x"` (non-object); `Object.assign(() => {}, { success: true, result: 1 })` (function); `{ result: 1 }` (no `success`); `{ success: "true", result: 1 }` (non-boolean `success`); `{ success: true }` (no `result`). PRED-05 accepts `{ success: false, result: { code: 1, message: "x" } }` and its prototype-chain form. PRED-06 rejects, one clause each: `{ success: true, result: { code: 1, message: "x" } }` (success not `false`); `{ success: false, result: null }` (null result); `{ success: false, result: "x" }` (non-object result); `{ success: false, result: Object.assign(() => {}, { code: 1, message: "x" }) }` (function result); `{ success: false, result: { code: "1", message: "x" } }` (non-number code); `{ success: false, result: { code: 1, message: 2 } }` (non-string message); and `Object.assign(() => {}, { success: false, result: { code: 1, message: "x" } })` (function envelope, rejected by way of PRED-04). An array is an object: an array carrying the required properties is accepted wherever an object is. |
