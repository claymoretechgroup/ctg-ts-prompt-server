...
| PRED-03 | `isCTGPromptServerResponse` | Returns `true` when `typeof value` is `"object"`, `value` is not `null`, `Array.isArray(value)` is `false`, `typeof value.success` is `"boolean"`, and `"result" in value` is `true` — so a `result` property holding `undefined` counts, and `success` and `result` inherited from the prototype chain both count. |
| PRED-04 | `isCTGPromptServerResponse` | Returns `false` for `null`, `undefined`, a non-object, an array (even one carrying `success` and `result` properties), an object without a `success` property or whose `success` is not a boolean, and an object for which `"result" in value` is `false`. |
...
| PRED-06 | `isCTGPromptServerErrorResponse` | Returns `false` for every value PRED-04 rejects, an object whose `success` is `true`, an object whose `result` is `null`, an array, or not an object, and an object whose `result.code` is not a number or whose `result.message` is not a string. |
...
| TEST-02 | PRED-03, PRED-04, PRED-05, PRED-06 | The predicates are driven with hand-built plain objects; no error instance and no HTTP call is involved. A success body such as `{ success: true, result: 1 }` is accepted by PRED-03 and rejected by PRED-06. The prototype case, `Object.create({ success: true, result: 1 })`, is accepted by PRED-03; the array case, `Object.assign([], { success: true, result: 1 })`, is rejected by PRED-04; the null-result case, `{ success: false, result: null }`, and the array-result case, `{ success: false, result: Object.assign([], { code: 1, message: "x" }) }`, are rejected by PRED-06. |
