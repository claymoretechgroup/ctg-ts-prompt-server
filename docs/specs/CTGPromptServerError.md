...
[CTGPromptServerRequestError](./CTGPromptServerRequestError.md)
...
When `data` is omitted, the constructor stores `null`. The spec keeps the
...
The spec treats errors as five related sources or states:
...
Error codes are plain integers. The spec does not export a dedicated error
